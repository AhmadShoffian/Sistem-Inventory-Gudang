  ...
  <tbody>
    @forelse ($barangs as $barang)
      <tr class="hover:bg-blueGray-50">
        ...
        <td class="px-6 py-4 border-t border-blueGray-100">
          <div class="flex items-center space-x-2">
              <a href="{{ route('staff.barang.show', $barang->id) }}" class="text-blue-500 hover:text-blue-700 text-sm" title="Detail">
                  <i class="fas fa-eye"></i>
              </a>
              
              <a href="{{ route('staff.barang.edit', $barang->id) }}" class="text-blue-500 hover:text-blue-700 text-sm" title="Edit">
                  <i class="fas fa-edit"></i>
              </a>

            <form method="POST" action="{{ route('staff.barang.destroy', $barang->id) }}" class="form-delete inline">
                @csrf
                @method('DELETE')
    ...
    @empty
        <tr>
            <td colspan="6"
                class="text-center text-blueGray-400 text-sm py-4">
